...
export class View extends EventEmitter {
  ...
  showRecentsList(data) {
    ...
    if (data.length) {
      ...
      data.forEach((item) => {
        ...
        const listItem = this.domElementConstructor.createLocationItem(
          item.location,
          item.id,
          'rec',
          [deleteBtn],
  ...
  createCurrentWeather(location) {
    ...
    const dailyTemp = this.domElementConstructor.createAvgTemp(
      location.main.temp_max,
      location.main.temp_min,
    );
    const conditionImg = this.domElementConstructor.createConditionImg(
      location.weather[0].icon,
      IMG_DOUBLE_SCALE,
    );
    const description = this.domElementConstructor.createDescription(
      location.weather[0].description,
    );
    const tempNow = this.domElementConstructor.createNowNFeelTemp(
      location.main.temp,
      location.main.feels_like,
    );
    const addTitleCont = this.domElementConstructor.createBoxElement(
      'div',
      ['title-box'],
      [date, dailyTemp, conditionImg, description, tempNow],
    );

    const wind = this.domElementConstructor.createAddParameter(
      'wind',
      'Wind speed',
      location.wind.speed,
      location.wind.deg,
    );
    const humidity = this.domElementConstructor.createAddParameter(
      'hum',
      'Humidity',
      location.main.humidity,
    );
    const pressure = this.domElementConstructor.createAddParameter(
      'pres',
      'Pressure',
      location.main.pressure,
    );
    const visibility = this.domElementConstructor.createAddParameter(
      'vis',
      'Visibility',
      (location.visibility / 1000).toFixed(1),
    );
    const addCont = this.domElementConstructor.createBoxElement(
      'div',
      ['add-box'],
      [wind, humidity, pressure, visibility],
    );

    const sunTime = this.domElementConstructor.createSunTime(
      this.convertTimeToUTC(location.sys.sunrise),
      this.convertTimeToUTC(location.sys.sunset),
    );

    return this.domElementConstructor.createBoxElement(
      'div',
      ['bg-cont'],
      [addTitleCont, addCont, sunTime],
    );
  }
  ...
  createWeatherForecastCard(data) {
    ...
    const leftPart = this.domElementConstructor.createBoxElement(
      'div',
      ['left-part'],
      [newDate, timeStamp, conditionImg, currTemp],
    );
